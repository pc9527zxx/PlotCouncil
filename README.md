<div align="center">
  ...
  <!-- Badges (Placeholders for style matching) -->
...
* [How to install / 安装指南](#how-to-install--安装指南)
  * [Frontend / 前端](#frontend--前端)
  * [Renderer / 渲染服务](#renderer--渲染服务)
...
<br> 前置要求：Node.js ≥ 20, Python ≥ 3.10
...
2. In the app **Settings** (UI), paste your **Gemini API Key**.
   <br> 在应用 **Settings**（界面）中粘贴您的 **Gemini API Key**。
...
* **No reviews appearing?**
  ...
  <br> 确保已输入有效的 API Key 并选择了 **Complex** 或 **Full** 模式。评审会在首次渲染后触发。
* **Blank plot or error?**
  <br> **空白图表或报错？**
  Check the **Logs** tab. If the plot is blank, switch to **Complex** mode so the Data Teacher can catch it.
  <br> 检查 **Logs** 标签页。如果是空白图，切换到 **Complex** 模式，让数据老师（Data Teacher）捕捉问题。
...
* The backend executes arbitrary Matplotlib code. **Deploy with caution** (Docker/Sandbox) if exposing to public.
  <br> 后端会执行任意 Matplotlib 代码。如果向公网开放，请**谨慎部署**（使用 Docker/沙箱）。
* Periodically clean `server/artifacts/` to reclaim disk space.
  <br> 定期清理 `server/artifacts/` 以回收磁盘空间。
