...
import { 
  Plus, Folder, Settings, FolderOpen,
  Pencil, Trash2, Cpu, ChevronDown, Check, Loader2, X, CheckSquare, Square,
  FolderPlus, ChevronRight, FolderInput, Minus, MinusSquare
} from 'lucide-react';
...
export const ProjectSidebar: React.FC<ProjectSidebarProps> = memo(({
  ...
  const renderProjectItem = (project: Project, inGroup = false) => {
    ...
    return (
      <div
        key={project.id}
        draggable={!selectMode && !isEditing}
      ...
      >
        ...
        {/* Folder icon */}
        {isActive && !selectMode ? (
          <FolderOpen className="shrink-0 w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
        ) : (
          <Folder className="shrink-0 w-3.5 h-3.5 text-slate-400 group-hover:text-slate-500" />
        )}
  ...
  return (
    <aside className="h-full w-full bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-300 flex flex-col overflow-hidden">
      {/* Header with actions */}
      <div className="flex items-center justify-between px-3 py-2 shrink-0 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-zinc-900/50">
      ...
      {selectMode && (
        <div className="px-3 py-1.5 border-b border-slate-200 dark:border-slate-800 bg-blue-50/50 dark:bg-blue-900/10 flex items-center gap-1.5 shrink-0">
          <button 
                ...
                {isEditingGroup ? (
                  ...
                  <>
                    ...
                    <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5">
                      {onRenameGroup && (
                        <button onClick={(e) => { e.stopPropagation(); startEditing(group.id, group.name, 'group'); }} className="p-0.5 text-slate-400 hover:text-indigo-600 rounded">
                          <Pencil className="w-3 h-3" />
                        </button>
                      )}
                      {onDeleteGroup && (
                        <button onClick={(e) => { e.stopPropagation(); onDeleteGroup(group.id); }} className="p-0.5 text-slate-400 hover:text-rose-500 rounded">
